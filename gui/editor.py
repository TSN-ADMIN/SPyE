...
from common.util import (
    get_char_pos, get_keypress, is_shown, is_word_at,
    me_, msg_box
)
from conf.debug import (
    DEBUG, dbg_DOCSTATE, dbg_EVENT, dbg_FILE,
    dbg_FOCUS, dbg_MOD_TYPE, dbg_POSITION_HISTORY,
    dbg_TIMER, dbg_whoami, dbg_method_calls
)
...
from const.editor import (
    NEWLINE, MSL_KEYS, PDB_KEYS, BRF_KEYS, CHARS, DOC_STYLE_HOTSPOT,
...
from const.macro import MACRO_CMDS
...
#NOTE, prevent circular dependency
        ...
        self.cph_cache_lst = [self.CurrentPos]
        # self.cph_cache_lst = [self.CurrentPos, (self.CurrentPos, self.CurrentPos)]
        self.cph_idx = 0  # cache index
        # self.cph_cache_lst = []
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#NOTE, EXPERIMENTAL CODE: _hover_scroll_lines
...
#NOTE, either BufferedDraw or Technology cause flicker when False
        ...
        self.SetMarginWidth(MGN['NUM'], sec['LineNumberWidth'] if mb.IsChecked(MI['MGN_NUM']) else 0)
        ...
        self.SetMarginWidth(MGN['SYM'], sec['SymbolWidth'] if mb.IsChecked(MI['MGN_SYM']) else 0)
        ...
        self.SetMarginWidth(MGN['FOL'], sec['FoldingWidth'] if mb.IsChecked(MI['MGN_FOL']) else 0)
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        ...
        img_siz = (16, 16)
        self.RegisterImage(1, PNG['open'].Bitmap)
...
###################################################################################################
    ...
    def binds(self):
        ...
        self.Bind(stc.EVT_STC_UPDATEUI, lambda e: self.set_x_scroll_offset())
...
#NOTE, was 'EVT_KEY_DOWN'; now works better with 'ESCAPE' key in 'SearchPanel'
        ...
        self.Bind(wx.EVT_SCROLLWIN_LINEDOWN, self.scrolling)
        self.Bind(wx.EVT_SCROLLWIN_LINEUP, self.scrolling)
        self.Bind(wx.EVT_SCROLLWIN_PAGEDOWN, self.scrolling)
        self.Bind(wx.EVT_SCROLLWIN_PAGEUP, self.scrolling)
        self.Bind(wx.EVT_SCROLLWIN_THUMBTRACK, self.scrolling)
        self.Bind(wx.EVT_MOUSE_CAPTURE_CHANGED, self.scrolling_release)
        self.Bind(wx.EVT_SCROLLWIN_THUMBRELEASE, self.scrolling_release)
        ...
        self.Bind(stc.EVT_STC_MARGINCLICK, self.margin_click_marker)
        self.Bind(stc.EVT_STC_MARGINCLICK, self.margin_click_fold)
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        # was: EVT_MOTION, EVT_SET_CURSOR only needs Ctrl (with 'NeedCtrlKey')
        self.Bind(wx.EVT_SET_CURSOR, self.colour_tooltip_set_delay)
        self.Bind(wx.EVT_TIMER, self.show_colour_tooltip)

        # was: EVT_MOTION which needs Ctrl AND mouse movement (with 'NeedCtrlKey')
...
#TODO, do not show msg at app startup, create flag for that
        # glb.SB.set_text('Folding style set to [%s]' % lbl)

        # glb.SB.push_text('Folding style set to [%s]' % lbl)
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
            ...
            if DEBUG['KBD'] > 1: print(txt)
            glb.SB.set_text(txt, 'AUX')
        else:
            for nam in BRF_KEYS:
                BRF_KEYS[nam] = 0
                if DEBUG['KBD'] > 1: print(txt % (nam, BRF_KEYS[nam]))
...
#TODO, revise DEBUG['KBD']
        # if DEBUG['KBD']: print(f'{me_()}')
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        if DEBUG['KBD'] > 2: dbg_EVENT(evt)
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#TODO, revise DEBUG['KBD']
        # if DEBUG['KBD']: print(f'{me_("F")}: [{nam:>3}] {kpr_sct}')
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@

        if not kpr_sct:
            return

        tlw, sch = glb.TLW, glb.SCH

#@@@----@@@@@
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#NOTE, EXPERIMENTAL CODE:
        # unhide menubar when hidden
        if evt.EventType == wx.EVT_KEY_UP.typeId:
            # print(f'[{kpr_sct}], [{self.prv_kpr_sct}]')
            # for c in self.prv_kpr_sct:
            #     print(ord(c), end=' ')
            if nam == chr(0) and self.prv_kpr_sct.startswith('Alt+'):
                if not glb.MB.IsAttached():
                    if DEBUG['KBD']: print(f'{me_("F")}: unhide menubar')
                    glb.TLW.SetMenuBar(glb.MB)
...
#@@@----@@@@@
        ...
        if kpr_sct in PDB_KEYS:
            ...
            if not tlw.debugger and cmd == 'start':
                if DEBUG['PDB']: print(dsc)
                tlw.debugger_start()
                # return
            elif tlw.debugger and cmd not in ['start', 'quit']:
                if DEBUG['PDB']: print(dsc)
                tlw.debugger.command(cmd)
                # return
            elif tlw.debugger and cmd == 'quit':
                if DEBUG['PDB']: print(dsc)
                tlw.debugger.quit()
                tlw.debugger = None
                # return
...
#NOTE, workaround: NO native multi caret movement support for Scintilla < 3.6 (wx < 4.1)
        if WX_VER < 4.1:
            ...
            if glb.CFG['MultiEdit']['Selection'] and self.Selections > 1 and kpr_sct in MSL_KEYS:
...
#FIX, 'slide into view' effect (hide), see 'def set_search_mode'
            sec = glb.CFG['PanelEffect']
            chc, dur = sec['Choice'], sec['Duration']
            sch.HideWithEffect(chc if glb.SPL['SCH'].swap else chc, dur)
            # tlw.Freeze()
            # glb.NBK.Freeze()
            # sch.Hide()
            # tlw.Thaw()
...
#@@@----@@@@@
        ...
        if glb.MB.IsChecked(MI['CRT_BRF']):
...
#NOTE, EXPERIMENTAL CODE:
        ...
        if any(c in kpr_sct for c in ('Alt+LEFT', 'Alt+RIGHT')):
            ...
            __move_sash(glb.SPN, glb.SPL['SPN'], 'LEFT', 100, pxl=pxl)

        if is_shown('RES'):
            if any(c in kpr_sct for c in ('Alt+UP', 'Alt+DOWN')):
                pxl = 200 if 'Ctrl' in kpr_sct else 50
                __move_sash(glb.SCH, glb.SPL['SCH'], 'UP', 275, pxl=pxl)
...
#HACK: prevent double '\n' when recording Enter key
        ...
        print(f'[{len(self.mac_lst):>3}]: {mac} -> {MACRO_CMDS[msg]}')

        glb.SB.set_text(f'Recording step {len(self.mac_lst)}')
        self.prv_lparam = lparam

    def margin_click_marker(self, evt):
        evt.Skip()
...
#TODO, integrate with 'toggle_bookmark'/'toggle_breakpoint'
        ...
        if msk & MRK[typ]['MSK']:
            ...
            glb.SB.set_text(f'{dsc} deleted from {lin + 1}')
        else:
            self.MarkerAdd(lin, MRK[typ]['NUM'])
            glb.SB.set_text(f'{dsc} added to {lin + 1}')
...
#TODO, integrate with 'toggle_bookmark'/'toggle_breakpoint'
        ...
        (ctl := gui.get_spt(typ).update_list(self))

        # if DEBUG['BMK']: print('  %14s = %s' % ('MARK', marker))

    def margin_click_fold(self, evt):
        evt.Skip()
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        ...
        # discard event when...
        if self.cph_active:  # jumping
            return
        if self.cph_scl_active:  # scrolling
            return
        if self.cph_sbw_active:  # symbol browsing
            return

        # update caret position history
        pos = self.CurrentPos
        self.cph_idx += 1
        self.cph_cache_lst.insert(self.cph_idx, pos)
        # self.cph_cache_lst.insert(self.cph_idx, (pos, self.GetSelection()))
        if DEBUG['CPH']: dbg_POSITION_HISTORY(self, 'pos_changed')

        if self.cph_idx < len(self.cph_cache_lst) - 1:
            self.cph_cache_lst = self.cph_cache_lst[:self.cph_idx + 1]
            if DEBUG['CPH']: dbg_POSITION_HISTORY(self, '   Cut MID')

        mxi = glb.CFG['CaretPositionHistory']['MaxItems']
        if len(self.cph_cache_lst) > mxi:
            self.cph_cache_lst = self.cph_cache_lst[-mxi:]
            if DEBUG['CPH']: dbg_POSITION_HISTORY(self, '   Cut MAX')
            self.cph_idx -= 1
...
#FIX, not used: obsolete?
    ...
    def scrolling_release(self, evt):
...
#HACK: regain focus when 'STC_MOD_CHANGE' condition below is True
        ...
        mod = evt.ModificationType  # stc change notification (modification event)

        # discard when style change has occurred
        if mod & stc.STC_MOD_CHANGESTYLE:
            # print(mod)
            return

        # discard when adding/removing indicator, e.g. mark_matches
        if mod & stc.STC_MOD_CHANGEINDICATOR:
            # print(mod)
            return
        ...
        if DEBUG['SME']: dbg_MOD_TYPE(evt, self)
...
#NOTE, CallAfter prevents infinite zoom handler loop
    ...
    def auto_complete(self, evt):
        ...
        if DEBUG['ACP']: print(f'{me_()}')
        if DEBUG['ACP'] > 1: dbg_EVENT(evt)
        # find word start
        cur = self.CurrentPos
        ...
        if DEBUG['ACP']: print('Typed chars:', typed_chars)
...
#INFO, URL=https://github.com/jacobslusser/ScintillaNET/wiki/Character-Autocompletion
        if DEBUG['AIB']: print(f'{me_()}')
        if DEBUG['AIB'] > 1: dbg_EVENT(evt)
...
#@@@@@@@@@@@@@@@@@@@
        ...
        if DEBUG['CTP']: print(f'{me_()}  [{pos=}]  [{self.fnm=}]')
        if DEBUG['CTP'] > 1: dbg_EVENT(evt)
        if DEBUG['CTP']: print(f' ***[{lin=}]  [{wrd=}]\n')
...
#FIX, calltip with dwell not working
    def calltip_end(self, evt):
        ...
        if DEBUG['CTP']: print(f'{me_()}    [{pos=}]  [{self.fnm=}]')
        if DEBUG['CTP'] > 1: dbg_EVENT(evt) ; print()
...
#HACK: current document only
    ...
    def mark_matches(self, evt=None, ind=0, fnd=''):
        ...
        if ind == 0 and not is_shown('RES'):
            if COMMON_EVENTS[evt.EventType] == 'EVT_STC_DOUBLECLICK':  # 10231
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        ...
        flg = sch.get_flags()
        lct = self.LineCount
        fil_reg = re.compile(rf'^  [^ ].*:$')  # filename pattern in 'search results'
        sel_lst = []
        # find and mark matches
        while True:
            pos = self.FindText(cur, max_pos, sel_txt, flg)
            if pos == stc.STC_INVALID_POSITION:
                break

            if DEBUG['IND']: print(f'        [{pos = }]')
...
#FIX, a binary file seems to loop forever, think: CRLF?
                    ...
                    print(f'[{fil_reg.search(self.GetLineText(lin)).span()!r:15}], {self.GetLineText(lin)}')
                    cur = self.PositionFromLine(lin) + self.GetLineLength(lin)
                    continue
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        ...
        if DEBUG['IND']: print(f'  [{cnt       = }]\n  [{sel_lst   = }]')

        dbg_TIMER('mark_mch', 'STOP')

        glb.SB.set_text(f'Found {cnt} occurrences of [{sel_txt}]')
...
#         self.mac_rec_active = False
    ...
    def macro_start(self, evt):
        ...
        self.mac_lst = []
        txt = 'Recording macro'
        if DEBUG['MAC']: print(txt)

    def macro_stop(self, evt):
        self.mac_rec_active = False
        self.StopRecord()

        if DEBUG['MAC']: print('Stop recording (%s steps)' % len(self.mac_lst))

#FIX, global macro recording? if not stop record at page change
        if evt:
            cnt = len(self.mac_lst)
            if not cnt:
                glb.SB.set_text('Empty recording', typ='ERROR')
            else:
                glb.SB.set_text('Recorded %s macro steps' % cnt, typ='WARN')
    ...
    def macro_TEST(self, evt):
        if DEBUG['MAC']: print('TEST macro message table...')
...
# self.SetSelectionMode(3)                   # (int selectionMode)
        ...
        # import collections
        # mcm = collections.OrderedDict(sorted(MACRO_CMDS.items()))
        ...
        if DEBUG['MAC']: print(self.macro_to_source(), '\nEnd TEST')

    def macro_play(self, evt):
        if DEBUG['MAC']: print('Playing macro...')
...
#TODO??
        for cmd in self.mac_lst:
            ...
            if DEBUG['MAC']: print('  [{}] [{}] [{}]'.format(msg, wparam, lparam if not '\n' else '\\n'))
...
################################################################
        ...
        src_lst.append(f'    print("Executed")')    #TEST
...
#FIX, consolidate language/styling process into 1 method/call:
#FIX, see 'update_language_styling', 'update_styling' and 'styling'
    def update_language_styling(self, lng_lst):
...
#TODO, styling BEGIN
        if not lng_lst:
            lng_lst = list()  # revert to plain text
            lng_lst.append(LANG[-1])
...
#NOTE, margins are GLOBAL now (commented)
    def dft_docstate(self):
        ...
        docstate = {
            ...
            # MI['MGN_SYM']: sym,
            # MI['MGN_FOL']: fol,
            MI['EDG_NON']: mb.IsChecked(MI['EDG_NON']),
            MI['EDG_BCK']: mb.IsChecked(MI['EDG_BCK']),
            MI['EDG_LIN']: mb.IsChecked(MI['EDG_LIN']),
            # MI['IND_IUS']: mb.IsChecked(MI['IND_IUS']),
            # MI['IND_TW1']: mb.IsChecked(MI['IND_TW1']),
            # MI['IND_TW2']: mb.IsChecked(MI['IND_TW2']),
            # MI['IND_TW3']: mb.IsChecked(MI['IND_TW3']),
            # MI['IND_TW4']: mb.IsChecked(MI['IND_TW4']),
            # MI['IND_TW5']: mb.IsChecked(MI['IND_TW5']),
            # MI['IND_TW6']: mb.IsChecked(MI['IND_TW6']),
            # MI['IND_TW7']: mb.IsChecked(MI['IND_TW7']),
            # MI['IND_TW8']: mb.IsChecked(MI['IND_TW8']),
        }
        dbg_DOCSTATE(self.fnm, docstate)
        return docstate

    def get_docstate(self):
        mb = glb.MB

        num = mb.IsChecked(MI['MGN_NUM'])
        sym = mb.IsChecked(MI['MGN_SYM'])
        fol = mb.IsChecked(MI['MGN_FOL'])
        docstate = {
            MI['DOC_WRP']: mb.IsChecked(MI['DOC_WRP']),
            MI['DOC_EOL']: mb.IsChecked(MI['DOC_EOL']),
            MI['DOC_WSP']: mb.IsChecked(MI['DOC_WSP']),
            MI['DOC_LCK']: mb.IsChecked(MI['DOC_LCK']),
            # MI['MGN_ALL']: all([num, sym, fol]),
            # MI['MGN_NUM']: num,
            # MI['MGN_SYM']: sym,
            # MI['MGN_FOL']: fol,
    ...
    def set_docstate(self):
        ...
        mb.Check(MI['DOC_WRP'], self.state[MI['DOC_WRP']])
        mb.Check(MI['DOC_EOL'], self.state[MI['DOC_EOL']])
        mb.Check(MI['DOC_WSP'], self.state[MI['DOC_WSP']])
        mb.Check(MI['DOC_LCK'], self.state[MI['DOC_LCK']])
        # mb.Check(MI['MGN_ALL'], self.state[MI['MGN_ALL']])
        # mb.Check(MI['MGN_NUM'], self.state[MI['MGN_NUM']])
        # mb.Check(MI['MGN_SYM'], self.state[MI['MGN_SYM']])
        # mb.Check(MI['MGN_FOL'], self.state[MI['MGN_FOL']])
        mb.Check(MI['EDG_NON'], self.state[MI['EDG_NON']])
        mb.Check(MI['EDG_BCK'], self.state[MI['EDG_BCK']])
        mb.Check(MI['EDG_LIN'], self.state[MI['EDG_LIN']])
        # mb.Check(MI['IND_IUS'], self.state[MI['IND_IUS']])
        # mb.Check(MI['IND_TW1'], self.state[MI['IND_TW1']])
        # mb.Check(MI['IND_TW2'], self.state[MI['IND_TW2']])
        # mb.Check(MI['IND_TW3'], self.state[MI['IND_TW3']])
        # mb.Check(MI['IND_TW4'], self.state[MI['IND_TW4']])
        # mb.Check(MI['IND_TW5'], self.state[MI['IND_TW5']])
        # mb.Check(MI['IND_TW6'], self.state[MI['IND_TW6']])
        # mb.Check(MI['IND_TW7'], self.state[MI['IND_TW7']])
        # mb.Check(MI['IND_TW8'], self.state[MI['IND_TW8']])
        docstate = self.state
        dbg_DOCSTATE(self.fnm, docstate)
        return docstate
...
#         bmk_lst = list(enumerate(bmk_lst, start=1))
        if DEBUG['BMK'] > 1: print('*** Get:', bmk_lst)
        return bmk_lst
...
#             src = bmk[3]
        ...
        if DEBUG['BMK'] > 1: print('*** Set:', bmk_lst)
        return bmk_lst
...
#         bpt_lst = list(enumerate(bpt_lst, start=1))
        if DEBUG['BPT'] > 1: print('*** Get:', bpt_lst)
        return bpt_lst
    ...
    def set_breakpoints(self, bpt_lst=None):
        ...
        if (ctl := gui.get_spt('BPT')):
#FIX, hack: used ONLY AT STARTUP, might integrate with 'update_list'
            # (ctl := gui.get_spt('BPT')).update_list(self)
            ctl.update_list_hack(self, bpt_lst)

        if DEBUG['BPT'] > 1: print('*** Set:', bpt_lst)
        return
...
#NOTE, maybe change to regex = "^(#[A-Fa-f0-9]{6})$"
        ...
        if not (len(wrd) == 7 and wrd.startswith('#')):
            ...
            r, g, b = (int('0x' + wrd[i:i + 2], 16) for i in (1, 3, 5))
...
#             dc.DrawRectangle(rct)
    ...
    def show_symbol_popup(self, evt):
        ...
        tlw = glb.TLW
        if not any(wrd in w for w in [tlw.dfn_idx, tlw.ref_idx, tlw.imp_idx, tlw.var_idx, tlw.wrd_idx, tlw.qts_idx]):
            return
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        ...
        if find_next:
            ...
            pos = self.FindText(cur, self.LastPosition, self.cur_sel_wrd, flg)
            if pos == stc.STC_INVALID_POSITION:
                ...
                #     print(pos)
                # else:
                    glb.SB.set_text('Unable to find [%s]' % self.cur_sel_wrd)
            else:
                self.AddSelection(pos + len(self.cur_sel_wrd), pos)
                self.EnsureCaretVisible()

    def set_x_scroll_offset(self):
        rlr = glb.RLR
...
###########################################################################
#NOTE, EXPERIMENTAL CODE: convert_indentation, create_indentation
###########################################################################
