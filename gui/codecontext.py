...
# copied from "Python\Lib\idlelib", (modified) module CodeContext.py
...
from const.lang import CCX_BLOCKOPENERS
import gui

getspacesfirstword = lambda s, c=re.compile(r'^(\s*)(\w*)'): c.match(s).groups()


class CodeContext(wx.Panel):
    ...
    def __init__(self, prt):
        ...
        self.stt_ccx = wx.StaticText(self, style=sty, name='sttCodeContext')
        self.stt_ccx.SetBackgroundColour(bgc)
        self.stt_ccx.SetForegroundColour(fgc)
        self.stt_ccx.SetFont(wx.Font(10, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD, faceName=fnt))

#TODO, box around code context?
#         self.stb = wx.StaticBox(self.stt_ccx, size=(900, 100), label='Code Context')
...
#NOTE, 1-based line numbering, STC is 0-based...
        ...
        self.Bind(wx.EVT_CONTEXT_MENU, lambda e: gui.ContextMenu(e, 'CCX'))
...
#INFO, URL=https://stackoverflow.com/questions/3420021/wxpython-how-to-make-a-textctrl-fill-a-panel
        ...
        box.Add(self.stt_ccx, 1, wx.EXPAND)
    ...
    def get_line_info(self, linenum):
        ...
        """
        txt = self.doc.GetLine(linenum - 1)
    ...
    def update(self, evt):
        ...
        if not glb.SPL['CCX'].IsSplit():  # code context not visible
            return
#         try:                                # catch empty document
        if not (doc := get_doc()): return
#         except TypeError:
#             return
        self.doc = doc
        new_topvisible = self.doc.FirstVisibleLine + 1
        ...
        self.stt_ccx.SetLabel(''.join(context_strings))
