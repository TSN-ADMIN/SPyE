...
from common.util import (
    d_type, funcname, is_shown, msg_box, not_implemented, me_, rs_,
    set_font, make_modal
)
from conf.debug import (
    DEBUG, dbg_FOCUS, dbg_TIMER, dbg_SIDEPANEL,
    dbg_call_stack, dbg_method_calls
)
from const.app import (
    APP, EXE, OPT_CTAGS, OUT_CTAGS, LOC, CLR
)
from const.common import TXT_NIL, LOREM_IPSUM
from const.editor import MRK
from const import glb
from const.lang import LANG, LANG_WILDCARDS
from const.sidepanel import SPT_TMPFIL_NAME, SPT, SPT_COL_HEADERS, TSK_TAGS
...
#NOTE, override 'ColumnSorterMixin' method from wx/lib/mixins/listctrl.py
...
# base list control
# @dbg_method_calls()
class ListCtrlTool(wx.ListCtrl, ListCtrlOverrides, ColSorter, AutoWidth, Alternate, mix.Help):
    ...
    def __init__(self, prt, key, sec, doc=None):
...
############################################################
        ...
        # self.bxv.Add(self.mnf_flt, 1, wx.EXPAND)
        # self.bxv.Add(self, 1, wx.EXPAND)
        # self.prt.SetSizer(self.bxv)
############################################################

        self._binds()

    def _binds(self):
        bind_hover(self, self.sec)
        if self.IsVirtual():
            self.Bind(wx.EVT_LIST_COL_CLICK, self.virtual_sort)
        self.Bind(wx.EVT_LIST_ITEM_SELECTED if glb.CFG[self.sec]['SingleClick'] else wx.EVT_LIST_ITEM_ACTIVATED, self.activate_item)
        self.Bind(wx.EVT_COMMAND_LEFT_CLICK, lambda e: dbg_FOCUS(glb.DOC) if glb.CFG[self.sec]['SingleClick'] else None)
...
#HACK: hide empty column 0
    ...
    def _populate_list(self, key, lbl, doc):
        ...
        if key not in ('PLT', 'PFL', 'PYS', 'VLT'):
            self.update_list(doc, lbl)

        # used by ColumnSorterMixin, see wx/lib/mixins/listctrl.py
        self.itemDataMap = self.rows

        cnt = len(self.rows)
#FIX, only show msg when any side panel tool visible
#NOTE, NOT working correctly
        if not any([is_shown(p) for p in SPT]):
            glb.SB.set_text(f'Found {cnt} {lbl} occurrences')

    def _add_item(self, row, clr, extra=None):
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    def add_item_proxy(self, row, clr, extra=None):
        if self.IsVirtual():
            self.vrt_rows.append(row)
        else:
            self._add_item(row, clr, extra)
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#TODO, share pre/post functionality in 'update_list'
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    def _pre_update_list(self, lbl):
        ...
        if self.key not in ('BPT'):
            dbg_call_stack(f'{me_("F")}')
            self.delete_all_items()
        #@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        return True
        ...

    def update_list(self, doc=None, lbl=TXT_NIL):
        if self._pre_update_list(lbl):
            self._update_list(doc)  # call subclass method
        self._post_update_list(lbl)

    def _post_update_list(self, lbl):
        # print(f'{me_("F"):23}  [{lbl.title()}]')
        self.virtual_set_item_count()
        ...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    # def _pre_activate_item(self, lbl):
    #     print(f'{me_("F"):23}  [{lbl.title()}]')
    #     ...

    # def activate_item(self, lbl, doc=None):
    #     self._pre_activate_item(lbl)
    #     self._activate_item(doc)  # call subclass method
    #     self._post_activate_item(lbl)

    # def _post_activate_item(self, lbl):
    #     print(f'{me_("F"):23}  [{lbl.title()}]')
    #     ...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    def show_list_filter_box(self):
        ...
        edt = glb.DOC  # NOW: glb.DOC, WAS: glb.NBK.CurrentPage
        edt_wid = edt.ClientSize[0]
        flt_wid = self.mnf_flt.ClientSize[0]
        self.mnf_flt.Move(edt.ClientToScreen(edt_wid - flt_wid - 25, 20))
        self.mnf_flt.Show()
        dbg_FOCUS(self.mnf_flt.txc_flt)
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    def _keypress(self, evt):
        ...
        glb.SPN.page_changed(None)  # force '_update_icons'
        dbg_FOCUS(self.doc)
    ...
    def _refresh_title(self):
        ...
        doc = glb.DOC  # NOW: glb.DOC, WAS: glb.NBK.CurrentPage
        self.SetTitle(f'[{doc.fnm}] {self.ttl}: {cnt:3d} {txt}')
        if DEBUG['LFL']: print(f'[{doc.fnm}] {self.ttl} = [{self.txc_flt.Value}]: {cnt:3d} {txt}')
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        ...
        pub.subscribe(self.page_changed, 'pag_chg_spn')
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
            if DEBUG['SPN']: print(f'{spt.lbl:11} filter = [{self.flt_dct[spt.lbl]}]')
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    def _tooltip(self, evt, show=True):
        ...
        self.chc.ttp += f'Key\tTool         \tFilter\n'
        self.chc.ttp += f'-----\t-------------\t-----------\n'
        # choice control tooltip
        for key, spt in SPT.items():
            if key in ('PRJ', 'FXP', 'SDF', 'DCM', 'MDN', 'CFW', 'SNP'):
                dsc = ' '
    ...
    def _update_icons(self, sel, key, ctl, cls, doc, siz=16):
        _SZR_ARG = (1, wx.LEFT | wx.EXPAND, 2)  # proportion, flags, border

        lbl = get_lbl(sel)
        flt_txt = self.flt_dct[lbl]

        # 'Pylint', 'Pyflakes', 'Pycodestyle' & 'Vulture' panels: 'Run'
        flg_run = key in ('PLT', 'PFL', 'PYS', 'VLT') and hasattr(cls, 'run')
        # list panels: 'FilterEdit'
        flg_lfe = key not in ('PRJ', 'FXP', 'SDF', 'DCM', 'MDN', 'CFW', 'SNP')
        # list panels: 'FilterClear'
        flg_lfc = flg_lfe and flt_txt

        if DEBUG['LFL']: print(f'{lbl=:11}, {flg_lfe=!r:>5}, {flg_lfc=!r:>5}, {flt_txt=}\n')

        # list & 'Symbol' panels: 'Refresh', 'Sort'
        flg_rfr = key not in ('PRJ', 'FXP', 'DCM')
        flg_srt = flg_rfr
        # all panels: 'Help'
        flg_hlp = True

        # destroy current buttons when app fully started
        if glb.APP.ready:
            for obj in (self.btn_run, self.btn_lfe, self.btn_lfc, self.btn_rfr, self.btn_srt, self.btn_hlp):
                self.bxh.Detach(obj)
                obj.Destroy()

        # tooltip filter text, help event, transparent button, widget, style
        ...
        _run = self.btn_run = wgt(self, wx.ID_ANY, PNG[f'run_{siz}' if flg_run else _no].Bitmap, style=sty(flg_run), name='icoRun')
        _lfe = self.btn_lfe = wgt(self, wx.ID_ANY, PNG[f'filter_edit_{siz}' if flg_lfe else _no].Bitmap, style=sty(flg_lfe), name='icoFilterEdit')
        _lfc = self.btn_lfc = wgt(self, wx.ID_ANY, PNG[f'filter_clear_{siz}' if flg_lfc else _no].Bitmap, style=sty(flg_lfc), name='icoFilterClear')
        _rfr = self.btn_rfr = wgt(self, wx.ID_ANY, PNG[f'refresh_{siz}' if flg_rfr else _no].Bitmap, style=sty(flg_rfr), name='icoRefresh')
        _srt = self.btn_srt = wgt(self, wx.ID_ANY, PNG[f'sort_{siz}' if flg_srt else _no].Bitmap, style=sty(flg_srt), name='icoSort')
        _hlp = self.btn_hlp = wgt(self, wx.ID_ANY, PNG[f'help_{siz}' if flg_hlp else _no].Bitmap, style=sty(flg_hlp), name='icoHelp')

        _run.SetToolTip(f'Run [{lbl}] command' if flg_run else None)
        _lfe.SetToolTip(f'Edit [{lbl}] list filter{flt_txt_ttp}' if flg_lfe else None)
        _lfc.SetToolTip(f'Clear [{lbl}] list filter{flt_txt_ttp}' if flg_lfc else None)
        _rfr.SetToolTip(f'Refresh [{lbl}] panel' if flg_rfr else None)
        _srt.SetToolTip(f'Restore [{lbl}] sort state' if flg_srt else None)
        _hlp.SetToolTip(f'Side Panel Tool help [{lbl}]' if flg_hlp else None)

        _run.Bind(wx.EVT_BUTTON, lambda e: cls.run(ctl, doc) if flg_run else None)
        _lfe.Bind(wx.EVT_BUTTON, lambda e: ctl.show_list_filter_box() if flg_lfe else None)
        _lfc.Bind(wx.EVT_BUTTON, lambda e: self.clear_filter() if flg_lfc else None)
        _rfr.Bind(wx.EVT_BUTTON, lambda e: ctl.update_list(doc) if flg_rfr else None)
        _srt.Bind(wx.EVT_BUTTON, lambda e: restore_sort_state(ctl) if flg_srt else None)
        _hlp.Bind(wx.EVT_BUTTON, lambda e: wx.PostEvent(get_spt(), hlp_evt)) if flg_hlp else None

        # repopulate/refresh buttons
        for idx, obj in enumerate((_run, _lfe, _lfc, _rfr, _srt, _hlp), start=1):
            self.bxh.Insert(idx, obj, *_SZR_ARG)
...
#HACK: resize function for 'Side Panel Tool' controls
    def _resize(self, evt, ctl, cbp):
        if DEBUG['GEN']: print(f'{ctl}: _resize')
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        if DEBUG['GLB']:
            print(f'  SPN: {id(glb.DOC) = } -> {glb.DOC.fnm = }')
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        ...
        if not ctl:
            if sgl:
                ...
                if hasattr((ctl := self.sgl_lst[sel]), 'update_list'):
                    ctl.update_list(doc)
            else:
...
#FIX, put choice book page panel in BoxSizer
        ...
        update_choice_labels()
        self._update_icons(sel, key, ctl, cls, doc)
        self.Layout()
...
class DocumentList(ListCtrlTool):
    ...
    def __init__(self, prt, doc):
        super().__init__(prt, 'DOC', 'Document')
        if DEBUG['DCL']: print(f'{me_()}')

    def _update_list(self, doc=None):
        for pag, doc in glb.NBK.open_docs():
            fnm, dnm, lng = doc.fnm, doc.dnm, doc.lng_nam
            if DEBUG['DCL']: print('    doc[%d] = [%s] [%s] [%s]' % (pag, fnm, dnm, lng))

            row = ('', fnm, dnm, lng)
            self.add_item_proxy(row, glb.CFG['Document']['ListRowAltColour'])
...
#FIX, share initial functionality with superclass implementation
        ...
        fnm = self.GetItemText(idx, 1)  # filename
        dnm = self.GetItemText(idx, 2)  # dirname
...
#NOTE, workaround: no further processing, now only handles Python source code
        if not is_python_code(self, doc):
            return
...
#HACK: though 'ProjectTree' is no list, use 'update_list' as core method here
...
class BookmarkList(ListCtrlTool):
    ...
    def __init__(self, prt, doc):
        super().__init__(prt, 'BMK', 'Bookmark', doc)
        ColSorter.SortListItems(self, col=1)  # sort by bookmark number
        if DEBUG['BMK']: print(f'{me_()}')
    ...
    def _update_list(self, doc):
        for bmk in doc.get_bookmarks():
            if DEBUG['BMK'] > 1: print('    bmk = [%s]' % str(bmk))

            row = ('', str(bmk[0]), str(bmk[1]), doc.GetLine(bmk[1] - 1).strip())
            self.add_item_proxy(row, glb.CFG['Bookmark']['ListRowAltColour'])
    ...
    def _activate_item(self, evt, idx):
        ...
        if DEBUG['BMK']: print(f'  [{self.GetItemText(idx, 1).rstrip()}] on line: {lin}')
    ...
    def delete_item(self, evt):
        ...
        if DEBUG['BMK']: print(f'{me_()}')
        idx = evt.Index
        if DEBUG['BMK']: print(f'  delete #{idx + 1}: [{self.GetItemText(idx, 3).rstrip()}]')
        lin = int(self.GetItemText(idx, 2))  # lineno
        glb.SB.set_text('Bookmark deleted from line %d' % (lin))
...
class LanguageList(ListCtrlTool):
    ...
    def __init__(self, prt):
        super().__init__(prt, 'LNG', 'Language')
        ColSorter.SortListItems(self, col=1)  # sort by lineno
        if DEBUG['SPN']: print(f'{me_()}')
...
#TODO, add 'enable selected lexer' for cur doc to '_activate_item'
    ...
    def _update_list(self, doc=None):
        for idx, (__, typ, nam, ext, __) in enumerate(LANG):
            if DEBUG['GEN']: print('    pfl = [%s] [%s] [%s] [%s]' % (idx, nam, typ, ext))
            row = ('', str(idx), nam, typ, ext)
            self.add_item_proxy(row, glb.CFG['Language']['ListRowAltColour'])
...
#FIX, selecting/hovering 'Perl' takes more time to execute...
        ...
        if DEBUG['GEN']: print(f'  [{num:2}]: [{nam:12}]  [{typ:10}]  [{ext:20}]')
...
#FIX, see 'update_language_styling', 'update_styling' and 'styling'
        # selected language
        doc.update_language_styling([[lng for lng in LANG][int(num)]])
...
#NOTE, workaround: no further processing, now only handles Python source code
        ...
        if glb.CFG['SymbolDef']['ShowIcons']:
            ...
            for nam in ('variable', 'unknown', 'namespace', 'class', 'member', 'function'):
                bmp_lst.append(PNG[f'tree_{nam}_16'].Bitmap)
...
#HACK: use private '__activate_item' instead of protected '_activate_item' in 'ListCtrlTool' subclasses
#NOTE, 'EVT_TREE_SEL_CHANGED' needs single click, EVT_TREE_ITEM_ACTIVATED needs double click
        self.Bind(wx.EVT_TREE_SEL_CHANGED if glb.CFG['SymbolDef']['SingleClick'] else wx.EVT_TREE_ITEM_ACTIVATED, self.__activate_item)
        self.Bind(wx.EVT_CONTEXT_MENU, lambda e: gui.ContextMenu(e, 'SDF'))
        self.Bind(wx.EVT_HELP, self.Help)
        bind_hover(self, 'SymbolDef')
...
#HACK: though 'SymbolTree' is no list, use 'update_list' as core method here
#FIX, when 'Refresh' called ('ico_rfr'), DESTROY CURRENT 'SymbolTree' instance
    def _update_list(self, doc=None):
        self.__init__(self.prt, doc)

#HACK: use private '__activate_item' instead of protected '_activate_item' in 'ListCtrlTool' subclasses
    def __activate_item(self, evt):
        if not (doc := get_doc()): return
...
#NOTE, workaround: 'RuntimeError: wrapped C/C++ object of type SymbolTree has been deleted'
        ...
        if DEBUG['GEN']: print(f'line {lin:4}: [{lbl}]')
        doc.GotoLine(lin - 1)
        doc.LineEndExtend()
        if glb.CFG['SymbolDef']['CentreCaret']:
            doc.VerticalCentreCaret()
#FIX, use 'wx.CallAfter(dbg_FOCUS, doc)' in bookmark/breakpoint/task controls, too
...
class MacroList(ListCtrlTool):
    ...
    def __init__(self, prt):
        super().__init__(prt, 'MAC', 'Macro')
        ColSorter.SortListItems(self, col=1)  # sort by macro name
        if DEBUG['SPN']: print(f'{me_()}')

    def _update_list(self, doc=None):
        mac = {
            'dummy 0': 'MACRO 0 description',
            'dummy 1': 'MACRO 1 description',
            'dummy 2': 'MACRO 2 description',
            'dummy 3': 'MACRO 3 description',
            'dummy 4': 'MACRO 4 description',
            'dummy 5': 'MACRO 5 description',
            'dummy 6': 'MACRO 6 description',
            'dummy 7': 'MACRO 7 description',
            'dummy 8': 'MACRO 8 description',
            'dummy 9': 'MACRO 9 description',
        }

        for nam, dsc in mac.items():
            if DEBUG['SPN']: print(f'    mac = [{nam}: {dsc}]')

            row = ('', nam, dsc)
            self.add_item_proxy(row, glb.CFG['Macro']['ListRowAltColour'])
...
class TaskList(ListCtrlTool):
    ...
    def __init__(self, prt, doc):
        super().__init__(prt, 'TSK', 'Task', doc)
        ColSorter.SortListItems(self, col=2)  # sort by lineno
        if DEBUG['TSK']: print(f'{me_()}')
    ...
    def _update_list(self, doc):
        ...
        for tsk in TSK_TAGS:
            ...
            while (pos := doc.FindText(fr, to, tsk, flg)) != stc.STC_INVALID_POSITION:
                ...
                if DEBUG['TSK']: print(f'    tsk = [{tsk:>6}] - [{lin:5d} /{col:2d}]')

                row = ('', tsk[1:], f'{lin + 1}', dsc, 'Reserved')
                self.add_item_proxy(row, glb.CFG['Task']['ListRowAltColour'])
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    def delete_item(self, evt):
        ...
        if DEBUG['TSK']: print(f'{me_()}')
        idx = evt.Index
        if DEBUG['TSK']: print(f'  delete #{idx + 1}: [{self.GetItemText(idx, 3).rstrip()}]')
        lin = int(self.GetItemText(idx, 2))  # lineno
        glb.SB.set_text('Task deleted from line %d' % (lin))
...
class BreakpointList(ListCtrlTool):
    ...
    def __init__(self, prt, doc):
        super().__init__(prt, 'BPT', 'Breakpoint', doc)
        ColSorter.SortListItems(self, col=1)  # sort by breakpoint number
        if DEBUG['BPT']: print(f'{me_()}')
    ...
    def _update_list(self, doc):
        ...
        for bpt in doc.get_breakpoints():
            if DEBUG['BPT'] > 1: print('    bpt = [%s]' % str(bpt))
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
            ...
            self.add_item_proxy(row, glb.CFG['Breakpoint']['ListRowAltColour'])

#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#FIX, hack: USED ONLY AT STARTUP, might integrate wih 'update_list'
    def update_list_hack(self, doc, bpt_lst):
        for bpt in bpt_lst:
            if DEBUG['BPT'] > 1: print('    bpt = [%s]' % str(bpt))
            bpn = bpt[0]
            ena = bpt[1]  # enabled flag
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    def _activate_item(self, evt, idx):
        ...
        if DEBUG['BPT']: print(f'  [{self.GetItemText(idx, 1).rstrip()}] on line: {lin}')
    ...
    def delete_item(self, evt):
        ...
        if DEBUG['BPT']: print(f'{me_()}')
        idx = evt.Index
        if DEBUG['BPT']: print(f'  delete #{idx + 1}: [{self.GetItemText(idx, 4).rstrip()}]')
        lin = int(self.GetItemText(idx, 3))  # lineno
        glb.SB.set_text('Breakpoint deleted from line %d' % (lin))
    ...
    def enable_item(self, evt):
        ...
        glb.SB.set_text(f'{txt}abled breakpoint {bpn} on line {lin}')
...
class ConfigList(ListCtrlTool):
    ...
    def __init__(self, prt):
        super().__init__(prt, 'CFG', 'Config')
        ColSorter.SortListItems(self, col=1)  # sort by config section/key
        if DEBUG['SPN']: print(f'{me_()}')

        # selection colours
        self.key_clr = glb.CFG['Config']['KeySelBackColour']
        self.sec_clr = glb.CFG['Config']['SectionSelBackColour']
    ...
    def _update_list(self, doc=None):
        ...
        for sec in glb.CFG:
            for key in glb.CFG[sec]:
                ...
                val = glb.CFG[sec][key]
                typ = d_type(val)
                if DEBUG['SPN']: print(f'    cfg = [{sec}], [{key}], [{typ}], [{val}], "desc"')

                row = ('', str(idx), sec, key, typ, str(val), 'desc')
                self.add_item_proxy(row, glb.CFG['Config']['ListRowAltColour'])
...
#NOTE, prevent circular dependency
        ...
        if res > 0:
            doc.SelectNone()
            doc.SetSelBackground(True, self.sec_clr)
            # section only
            if res == 1:
            ...
            # both section and key
            elif res == 2:
                doc.SetAdditionalSelBackground(self.key_clr)
                for sel in range(2):
                    lin, sel_fnc = (lin_sec, doc.SetSelection) if sel == 0 else (lin_key, doc.AddSelection)
...
class HelpList(ListCtrlTool):
    ...
    def __init__(self, prt):
        super().__init__(prt, 'HLP', 'Help')
        ColSorter.SortListItems(self, col=1)  # sort by help name
        if DEBUG['SPN']: print(f'{me_()}')

    def _update_list(self, doc=None):
        hlp = {
            'dummy 0': 'HELP 0 description',
            'dummy 1': 'HELP 1 description',
            'dummy 2': 'HELP 2 description',
            'dummy 3': 'HELP 3 description',
            'dummy 4': 'HELP 4 description',
            'dummy 5': 'HELP 5 description',
            'dummy 6': 'HELP 6 description',
            'dummy 7': 'HELP 7 description',
            'dummy 8': 'HELP 8 description',
            'dummy 9': 'HELP 9 description',
        }

        for nam, dsc in hlp.items():
            if DEBUG['SPN']: print(f'    hlp = [{nam}: {dsc}]')

            row = ('', nam, dsc)
            self.add_item_proxy(row, glb.CFG['Help']['ListRowAltColour'])
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    def _update_list(self, doc):
        for lin_txt in self.out:
            ...
            if DEBUG['SPN']: print(f'    plt = [{lin}], [{col}], [{cat}], [{id_}], [{sym}], [{obj}], [{msg}]')

            row = ('', str(lin), str(int(col) + 1), cat, id_, sym, obj, msg)
            self.add_item_proxy(row, glb.CFG['Pylint']['ListRowAltColour'])
    ...
    def _activate_item(self, evt, idx):
        ...
        if DEBUG['GEN']: print(f'  [{self.GetItemText(idx, 1).rstrip()}] on line: {lin}')
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    def __init__(self, prt, doc):
        super().__init__(prt, 'PFL', 'Pyflakes')
        ColSorter.SortListItems(self, col=1)  # sort by lineno
        if DEBUG['SPN']: print(f'{me_()}')
...
#HACK: remove 'filename:' and replace 'col ' w/ 'col:' in Pyflakes output
        ...
        for lin_txt in self.out:
            ...
            if DEBUG['SPN']: print(f'    pfl = [{lin}], [{col}], [{msg}]')

            row = ('', str(lin), str(int(col)), msg)
            self.add_item_proxy(row, glb.CFG['Pyflakes']['ListRowAltColour'])
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    def __init__(self, prt, doc):
        super().__init__(prt, 'PYS', 'Pycodestyle')
        ColSorter.SortListItems(self, col=1)  # sort by lineno
        if DEBUG['SPN']: print(f'{me_()}')
    ...
    def _update_list(self, doc):
        for lin_txt in self.out:
            ...
            if DEBUG['SPN']: print(f'    pys = [{lin}], [{col}], [{id_}], [{msg}]')

            row = ('', str(lin), str(int(col)), id_, msg)
            self.add_item_proxy(row, glb.CFG['Pycodestyle']['ListRowAltColour'])
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        ...
        dbg_TIMER('pycodestyle', 'STOP')


class VultureList(ListCtrlTool):

# SPyE.py:426: unused import 'is_shown' (90% confidence)
# SPyE.py:431: unused import 'dbg_method_calls' (90% confidence)
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    def __init__(self, prt, doc):
        super().__init__(prt, 'VLT', 'Vulture')
        ColSorter.SortListItems(self, col=1)  # sort by lineno
        if DEBUG['SPN']: print(f'{me_()}')
    ...
    def _update_list(self, doc):
        for lin_txt in self.out:
            ...
            if DEBUG['SPN']: print(f'    vlt = [{lin}], [{msg}]')

            row = ('', str(lin), msg)
            self.add_item_proxy(row, glb.CFG['Vulture']['ListRowAltColour'])
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#NOTE, EXPERIMENTAL CODE: pycodestyle => 'PYS'
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
#INFO. a welcome bonus functionality ;-)
    ...
    return obj.Bind(wx.EVT_MOTION, lambda e: hover_select_list_item(e, obj, sec, flg) if val else None)
...
def cmd_output_to_list(ctl, cmd):
    ...
    ctl.update_list(None)
    restore_sort_state(ctl)


#DONE, created generic function for wx.ListCtrl and wx.TreeCtrl
#DONE,     with signature: hover_select_list_item(evt, obj, cfg_sec, flg)
#DONE,               e.g.: hover_select_list_item(evt, self, 'Bookmark', wx.LIST_HITTEST_ONITEMLABEL)
...
def hover_select_list_item(evt, obj, cfg_sec, flg):
    evt.Skip()
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#NOTE, EXPERIMENTAL CODE: avoid update when 'ListCrl item (wx.ListItem)' is already selected (e.g. 'wx.LIST_STATE_SELECTED')
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        if DEBUG['GEN']:
...
#FIX, 'glb.SPN' used instead of 'spn=' parm (not used anymore)
def get_spt(key=None, spn=None, doc=None):
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    if not doc:
        doc = get_doc()
    if key in SPT:
...
#NOTE, workaround: no further processing, now only handles Python source code
def is_python_code(obj, doc):
    if doc.lng_typ != 'python':
        stt = wx.StaticText(obj, label='\n WORKAROUND: lng_typ not Python! \n')
...
def update_choice_labels():
    ...
    if DEBUG['LFL']: print(f'*      File = [{doc.fnm}]')
...
#FIX, needs better coding...
        ...
        if key not in ('DOC', 'BMK', 'LNG', 'MAC', 'TSK', 'BPT', 'DBG', 'CFG', 'HLP', 'PLT', 'PFL', 'PYS', 'VLT'):
            if DEBUG['LFL']: print(f'{spt.idx:2}', '=> skip', key)
            continue
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@

        flt_txt = spn.flt_dct[spt.lbl]

        if (ctl := get_spt(key, spn, doc)):
            # update 'FilterBox'
        ...
        if DEBUG['LFL']: print(f'{spt.idx:2}', spt)
