...
from common.util import (
    hex_colour, is_shown, me_, msg_box, not_implemented, set_icon
)
...
#NOTE, margins are GLOBAL now
        ...
        for __, doc_mgn in glb.NBK.open_docs():
            for mni in items:
                glb.MB.Check(mni, bool(chk))
            for typ, wid in types.items():
                doc_mgn.SetMarginWidth(typ, wid if chk else 0)
        if DEBUG['ZOO']:
            print(f'num {num:3d}, sym {sym:3d}, fol {fol:3d}, tot {tot:3d}  (ruler offset: all margins)')
    ...
    def view_auto_scroll(self, evt):
        ...
        while True:
            ...
            fnc()
            doc.Update()
            doc.scrolling(evt)  # force top line tooltip update
            _gks = wx.GetKeyState
            if _gks(wx.WXK_DOWN):
                fnc = doc.LineScrollDown
            elif _gks(wx.WXK_UP):
...
#NOTE, margins are GLOBAL now
        ...
        # glb.MB.Check(id_, evt.IsChecked())
        # same margins in all documents
        for __, doc_mgn in glb.NBK.open_docs():
...
#FIX, save settings to config, per doc/GLOBAL?
    ...
    def view_side_panel_tool(self, evt):
        ...
        glb.SPN.SetSelection(evt.Id - int(MI['SPT_DCL']))
