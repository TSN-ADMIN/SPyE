...
from common.util import (
    ColourToolTip, curdoc, drop_shadow, msg_box, not_implemented, rs_,
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#NOTE, EXPERIMENTAL: P a g e   T a b   H i s t o r y - BEGIN
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
#FIX, move 'page_tab_history_build' (restore) to end of 'apply()' in 'config.py'...?
    def page_tab_history_build(self, evt):
        ...
        DBG('PTH>1', f'   Build page tab history cache from open tabs (MRU = current page [{self.Selection}]):')
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#FIX, >>>>>>> D E B U G   T H I S ! ! <<<<<<<
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
            print('page_tab_history_build')
            doc = self.GetPage(pag)  # .txt1
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#FIX, >>>>>>> D E B U G   T H I S ! ! <<<<<<<
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    def page_tab_history_init(self, evt):
        ...
        DBG('PTH>1', f'  {me_("F")}    (page tab {self.current_page(_fwd)}): Tab [{_tab!r:>5}]  Fwd [{_fwd!r:>5}]  Key [{self.pth_nav_key}]')
...
#HACK, prevent selecting sequence of page tabs instead of just one
        ...
        if not (ctrl or shift):
            DBG('PTH>1', '=' * 46)
            DBG('PTH>1', f'  Ctrl/Shift released (MRU = page tab {self.Selection})')
            DBG('PTH>1', '=' * 46)
    ...
    def page_changing(self, evt):
        ...
        if self.glc_active:
            DBG('GEN', f'{me_("C, F")}: {self.glc_active = }')
            evt.Veto()
            return

        if not self.pth_active or 'Page' in self.pth_nav_key:
            DBG('PTH>1', f'    {me_("F")}: Allow on page tab {self.Selection}', evt)
        else:
            DBG('PTH>1', f'    {me_("F")}: Veto on page tab {self.Selection}')
            evt.Veto()
            return
...
#FIX, global macro recording? if not stop record at page change
    ...
    def page_changed(self, evt):
        glb.DOC = self.GetPage(self.Selection)  # .txt1

#FIX, replace this statement with 1 '@curdoc()' above 'page_changed'
#NOTE, NOW gives this ERROR when FIX is done
#INFO,   Traceback (most recent call last):
#INFO,     File "D:\Dev\D\wx\TSN_SPyE\dev\spye\src\gui\notebook.py", line 257, in page_tab_history_build
#INFO,       doc = self.GetPage(pag).txt1
#INFO,   TypeError: AuiNotebook.GetPage(): argument 1 has unexpected type 'str'
        if not (doc := glb.DOC): return

        glb.DOC = self.GetPage(self.Selection)

        DBG('GLB', f'  NBK: {id(glb.DOC) = } -> {glb.DOC.fnm = }')
        ...
        DBG('AFE', f'{" "*11}Includes: {me_("F")}\n')

#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#NOTE, EXPERIMENTAL: P a g e   T a b   H i s t o r y - END
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#FIX, crashes Python when StatusBar context menu [None] clicked
...
#NOTE, solves unresponsive page tab
        ...
        if self.sec_pth['Enable']:
            ...
            for key in reversed(list(self.pth_cache)):
                pnm = self.GetPage(key).pnm  # .txt1.pnm
                if key == evt.Selection:
                    self.pth_cache[evt.Selection] = pnm
                else:
                    self.pth_cache[key] = pnm
...
#NOTE: convenience generator function, yields pagenum and document (Editor) object
    ...
    def update_page_tab(self, doc, newtab=False):  # , spl=None):
...
#NOTE, prevent circular dependency
        ...
        if newtab:
            ...
            # if spl:
            #     self.insert_page_tab(pag, spl, fnm, bitmap=ico)
            # else:
    ...
    def update_ui(self, doc, full=False):
        ...
        if full:
            DBG('APP==2', f'NBK: {glb.APP.ready = }   {self.Selection:3}) {doc.fnm}')
    ...
    def make_tab_visible(self):
        ...
        DBG('NBK', f'==>> {me_("F")}: Active tab [{sel:3}]: visible=[{vis!r:>5}], caption=[{tab.caption}]')
...
#HACK, enable access to 'wx.aui.AuiTabCtrl' (tab control window)
    def tabctrl_access(self, evt):
        ...
        for _tc_id_ in range(TABCTRL_ID, TABCTRL_ID + 20):
            ...
            _tabctrl_found = bool(_tc_obj)

            # tab control has page tabs
            if pag_cnt:
                DBG('NBK>1', f'{_tc_id_}: {pag_cnt} pages in {_tc_obj} at pos: {_tc_obj.Position}')
#FIX, use enumerate, cnt
                cnt = 0
                # documents in this tab control
                for pag, doc in self.open_docs():
                    if cnt >= pag_cnt:
                        break
                    ...
                    if cnt == self.Selection:
                        DBG('NBK>1', f'{wsp}[CUR] [{txt}]')
                    else:
                        DBG('NBK>1', f'{wsp}       {txt}')
                    cnt += 1
            elif _tc_obj and not pag_cnt:
                DBG('NBK>1', f'{_tc_id_}: {pag_cnt} pages in {_tc_obj}')
            # else:
                # DBG('NBK', f'{_tc_id_}: non-existent tab control id')
    ...
    def tabctrl_hover(self, evt):
        ...
        if hit:
            ...
            if not doc.glc_obj:
                DBG('GEN', f'{me_("C, F")}: {pag=}, {doc.fnm=}, {doc}')
