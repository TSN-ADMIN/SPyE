...
from common.util import hex_colour, msg_box
...
from const.lang import FACES, LANG, LANG_KWRD, LANG_PRVW
...
#INFO, URL=https://groups.google.com/forum/#!topic/wxpython-users/F1PrSirxOBc
    ...
    def get_language(self, evt):
        ...
        if elm_nam_lst:
            ...
            # keywords
            for __, kws, lst in LANG_KWRD[self.lexer]:
...
#TODO, review code: default font/size, set globally?
    ...
    def style_emphasis(self, evt):
        obj = evt.EventObject
        DBG('LNG', f'[{obj.Name}] = [{obj.IsChecked()}]')
        setattr(self, obj.Name, obj.Name if obj.IsChecked() else '')
        self._update_preview()

    def style_font(self, evt):
        obj = evt.Font
        DBG('LNG', f'[face,size] = [{obj.FaceName},{obj.PointSize}]')
    ...
    def _join_stylespec(self):
        sty_lst = []
        if self.fore:
            sty_lst.append(f'fore:{self.fore}')
        if self.back:
            sty_lst.append(f'back:{self.back}')
        if self.bold:
        ...
        if self.face:
            sty_lst.append(f'face:{self.face}')
        if self.size:
            sty_lst.append(f'size:{self.size}')
    ...
    def _update_preview(self, flg=True):
        ...
        if flg:
            DBG('LNG', ' CUR:', self.cur[self.lang][self.elm_lst])
            DBG('LNG', ' WRK:', self.wrk[self.lang][self.elm_lst])
    ...
    def _reset_work(self):
        DBG('LNG', ' CUR:', self.cur, '\n-----')
        DBG('LNG', ' WRK:', self.wrk, '\n-----')
