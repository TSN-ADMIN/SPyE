...
#HACK, remove default close (X) button
        ...
        self.ibr.ShowMessage(TXT_NIL, wx.ICON_NONE)

        # search filter box
        self.txc_flt = txc_flt = wx.SearchCtrl(self, wx.ID_ANY, pos=(5, 15), size=(175, 25))
...
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    def on_exit(self, evt=None):
        ...
        if self.txc_flt.Value:
            ...
            txt = '  Filter Cleared: displaying all sections'
            self.ibr.ShowMessage(txt, wx.ICON_NONE)
            # wx.Bell()
            # self.tmr_ibr.StartOnce(2500)
            return
...
def get_widget_name(typ):
    # get widget name from section key spec (data type)
    wgt, pfx = None, 'wx'
    if typ in 'boolean':
        wgt = 'CheckBox'
    elif typ in 'colour':
...
def get_widget_value(wgt):
    ...
    if any(fnc(wgt) for fnc in (is_cbb, is_cbx, is_spc, is_txc)):
        ...
        # DBG('PRF', wgt.SelectedFont.FaceName)
