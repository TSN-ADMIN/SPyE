...
#INFO, copied from UliPad, (modified) module 'RulerCtrl.py'
...
class Ruler(wx.Panel, mix.Help):
    ...
    def set_column(self, col):
        if not self.sec_spl['LiveUpdate']:
            return
        ...
        def __draw_caret(x, y, siz, typ, clr):
            ...
            DBG('RLR', f'ruler (caret type): [{typ}]')

            if 'TRIANGLE' in typ:
                for i in range(siz):
                    DBG('RLR', f'  [{x - i:3}, {y - i:2}] -> [{x + i:3}, {y - i:2}]')
...
#############################
    ...
    def set_offset(self, offset):
        ...
        if self.offset != offset:
            DBG('RLR', f'  ruler (delta offset): {self.offset - offset:+5}')
            self.offset = offset
            self.Refresh()
