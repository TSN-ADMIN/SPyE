...
#    0:lexer (id)            1:lng_typ     2:lng_nam       3:extension(s)         4:lng_mni (id)           # 5:lng_ctg
LANG = (                                                                                           # [map CTags language name]
    (stc.STC_LEX_BASH,       'shell',      'Bash/Shell',   'csh|ksh|sh|zsh',      MI['LNG_BASH'],),        # 'Sh',),
    (stc.STC_LEX_BATCH,      'batch',      'Batch/CMD',    'bat|cmd',             MI['LNG_BATCH'],),       # 'DosBatch',),
    (stc.STC_LEX_CONF,       'config',     'Config',       'cfg|conf|ini|rc',     MI['LNG_CONF'],),        # 'Iniconf',),        # apache: conf
    (stc.STC_LEX_CPP,        'cpp',        'C/C++',        'c|cpp|cxx|h|hpp|hxx', MI['LNG_CPP'],),         # 'C++',),
    (stc.STC_LEX_CSS,        'css',        'CSS',          'css',                 MI['LNG_CSS'],),         # 'CSS',),
    (stc.STC_LEX_HTML,       'html',       'HTML',         'html|htm',            MI['LNG_HTML'],),        # 'HTML',),
    (stc.STC_LEX_MARKDOWN,   'markdown',   'Markdown',     'md',                  MI['LNG_MARKDOWN'],),    # 'Markdown',),
    (stc.STC_LEX_PASCAL,     'pascal',     'Pascal',       'pas|pp|inc',          MI['LNG_PASCAL'],),      # 'Pascal',),
    (stc.STC_LEX_PERL,       'perl',       'Perl',         'pl|pm|pod',           MI['LNG_PERL'],),        # 'Perl',),
    (stc.STC_LEX_PHPSCRIPT,  'php',        'PHP',          'php',                 MI['LNG_PHPSCRIPT'],),   # 'PHP',),
    (stc.STC_LEX_POWERSHELL, 'powershell', 'PowerShell',   'ps1',                 MI['LNG_POWERSHELL'],),  # 'PowerShell',),
    (stc.STC_LEX_PROPERTIES, 'properties', 'Properties',   'properties',          MI['LNG_PROPERTIES'],),  # 'JavaProperties',),
    (stc.STC_LEX_PYTHON,     'python',     'Python',       'py|pyw',              MI['LNG_PYTHON'],),      # 'Python',),
    (stc.STC_LEX_RUBY,       'ruby',       'Ruby',         'rb',                  MI['LNG_RUBY'],),        # 'Ruby',),
    (stc.STC_LEX_SQL,        'sql',        'SQL/(PL/SQL)', 'pls|pks|pkb|pck|sql', MI['LNG_SQL'],),         # 'SQL',),
    (stc.STC_LEX_TCL,        'tcl',        'TCL',          'tcl',                 MI['LNG_TCL'],),         # 'Tcl',),
    (stc.STC_LEX_XML,        'xml',        'XML',          'xml',                 MI['LNG_XML'],),         # 'XML',),
    (stc.STC_LEX_YAML,       'yaml',       'YAML',         'yaml|yml',            MI['LNG_YAML'],),        # 'Yaml',),
    (stc.STC_LEX_NULL,       'text',       'Plain Text',   'txt',                 MI['LNG_NULL'],),        # None,),
)
...
LANG_KWRD = {
    ...
    stc.STC_LEX_PYTHON: (
        ('Keywords', 0,
        ' '.join(keyword.kwlist)),  # + ' self doc mbr tbr sbr'
