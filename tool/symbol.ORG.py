...
#HACK: map hidden cancel button to ESCAPE key
        ...
        self.lbx_sym = wx.ListBox(self, choices=lbx_chc_lst, size=(330, 250), style=wx.LB_OWNERDRAW)
    ...
    def on_filter(self, evt):
        ...
        # highlight/select symbol name
        if self.lbx_sym.Count > 0:
            self.lbx_sym.SetSelection(0)
            self.on_select(None)

        # resize listbox
        if self.lbx_sym.Count < 10:
            self.lbx_sym.SetSize(330, self.lbx_sym.Count * 26)
        else:
