...
from common.util import drop_shadow, get_char_pos, funcname
from conf.debug import dbg_FOCUS, dbg_TIMER, dbg_method_calls
...
# @dbg_method_calls()
class DocMap(gui.Editor):

    __slots__ = ['prt', 'doc', 'zone', 'sec', 'drg_pos_map', 'drg_pos_zone' ]

    def __init__(self, prt, doc, **kwargs):
        self.sec = glb.CFG['DocMap']
...
#DONE, hack: UNKNOWN CAUSE OF BIND CONFLICT (oops ... inheriting from 'Editor')
        self.binds_hack()  # '_hack' suffix prevents conflict w/ 'Editor.binds'
    ...
    def binds_hack(self):
        ...
        self.Bind(wx.EVT_SET_FOCUS, lambda e: dbg_FOCUS(self.doc))
...
# @dbg_method_calls(exclude=['update'])
