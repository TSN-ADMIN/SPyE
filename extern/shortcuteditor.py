...
# copied from "Python\Lib\site-packages\wx\lib\agw", (modified) module shortcuteditor.py
...
#TSN, # from . import hypertreelist as HTL

from wx.lib.agw import genericmessagedialog as GMD
#TSN, # from . import genericmessagedialog as GMD
...
class HTMLHelpWindow(wx.Frame):
    ...
    def BuildToolBar(self, toolbar):
        ...
        toolbar.AddTool(wx.ID_BACKWARD, _('Back'), _html_back.GetBitmap(), wx.NullBitmap, wx.ITEM_NORMAL, _('Back'),
                        _('Go to the previous page'))

        toolbar.AddTool(wx.ID_FORWARD, _('Forward'), _html_forward.GetBitmap(), wx.NullBitmap, wx.ITEM_NORMAL, _('Forward'),
                        _('Go to the next page'))

        toolbar.AddSeparator()

        toolbar.AddTool(wx.ID_HOME, _('Home'), _html_home.GetBitmap(), wx.NullBitmap, wx.ITEM_NORMAL, _('Home Page'),
                        _('Go to the home page'))

        toolbar.AddTool(wx.ID_REFRESH, _('Refresh'), _html_reload.GetBitmap(), wx.NullBitmap, wx.ITEM_NORMAL, _('Refresh'),
                        _('Refresh the current page'))

        toolbar.AddSeparator()
        toolbar.AddStretchableSpace()

        toolbar.AddTool(wx.ID_PRINT, _('Print'), _html_print.GetBitmap(), wx.NullBitmap, wx.ITEM_NORMAL, _('Print'),
                        _('Print the current page'))
    ...
    def ToMenuItem(self, menuBar):
        ...
        if isinstance(menuItem, wx.Menu):
            ...
            if self.accelerator == DISABLED_STRING:
                ...
                label = f'{self.label}\t{self.accelerator}'
            ...
            if self.accelerator != DISABLED_STRING:
                label = f'{label}\t{self.accelerator}'
...
class ShortcutEditor(wx.Dialog):
    ...
    def FromAcceleratorTable(self, accelTable):
        ...
        for text, modifier, accel, ids in accelTable:
            ...
            shortcut = (modifier and [f'{modifier}+{accel}'] or [accel])[0]
...
#TSN, pprint @def ToAcceleratorTable
